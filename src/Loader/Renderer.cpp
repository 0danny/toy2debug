...
int32_t Renderer::init()
{
	...
	m_hWnd = CreateWindow(kAppTitle.data(), kAppTitle.data(), WS_POPUP, kWindowX, kWindowY, kWindowWidth, kWindowHeight, NULL, NULL, wc.hInstance, NULL);
...
void Renderer::run()
{
	...
	while (msg.message != WM_QUIT)
	{
		...
		bool isMouseInTitleBar = mousePos.x >= titleBarStart.x && mousePos.x <= titleBarEnd.x && mousePos.y >= titleBarStart.y && mousePos.y <= titleBarEnd.y;
		...
		if (isDragging)
		{
			if (ImGui::IsMouseDown(ImGuiMouseButton_Left))
			{
				POINT cursorPos;
				GetCursorPos(&cursorPos);
				SetWindowPos(m_hWnd, NULL, cursorPos.x - dragOffset.x, cursorPos.y - dragOffset.y, 0, 0, SWP_NOSIZE | SWP_NOZORDER);
			}
			else
			{
				isDragging = false;
